...
view address ({state, drawingSize} as model) =
  case state of
      ...
      Won p -> flow outward [drawModel address model,container drawingSize drawingSize middle <| bigRedText ((toString p)++ " won.")]
      Stalemate -> flow outward [drawModel address model,container drawingSize drawingSize middle <| bigRedText "Stalemate"]
      NotStarted -> startView address


startView: Signal.Address SimpleTTT.UAction -> Element
startView address = container 700 500 middle <|
                    flow down [Graphics.Element.size 250 80 <| button (message address SimpleTTT.SelectStart) "Start", --clickable Graphics.Element.Element (message address SimpleTTT.SelectStart) <| centered <| Text.color red <| Text.height 120 (fromString "Start"),
...
drawModel address {drawingSize, board,lastPlay} = let highlighted =  case lastPlay of
                                                  Nothing -> \_ _ -> True
                                                  Just n -> \x y -> n == x*3+y
                          in
                          ...
                          flow outward
                                ...
                                , flow down <| List.map (\i -> flow right <| toList <|
                                   indexedMap (\j c -> SimpleTTT.view address (highlighted i j) c) (slice (i * 3) (i * 3 + 3) board)) [0..2]
                                ]
